...
public class Packman extends Frame implements KeyListener, Runnable {
	...
	public Packman() {
		...
		for(int i=0; i<fx.length; i++) {
			fx[i] = (int)(Math.random()*461+20); //20~480
	...
	public void paint(Graphics g) {
		...
		//먹이
		for(int i=0; i<fx.length; i++) {
			g.drawImage(img_food, fx[i], fy[i], this);  //먹이생성
			if( x+25 <= fx[i]-10 &&  y+25 >= fy[i]-10 
			&&  x+25 >= fx[i]+30 &&  y+25 <= fy[i]+30 ) {
				fx[i] = -100;
				fy[i] = -100;
			}
		}
	...
	public void run() {
		while(true) {
			...
			repaint(); //기존의 paint된걸 버리고 다시 그리는건 여기서 해줘야 함
		}
	}
