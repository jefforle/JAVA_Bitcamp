...
///	private List<ShapeDTO> list; ///List가 부모라 가능
///	Object bo = new String();//부모로 접근하고 사용은 자식으로 해주는 경우가 많음
...
//익명이너클래스, can에서 이벤트가 발생 주의
		can.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) { 
				x2T.setText(e.getX()+""); //getX()값을 x2T에게 줘라
				y2T.setText(e.getY()+"");
...
/// DTO안에 도형정보가 저장됨		
				...
				dto.setX2(Integer.parseInt(x2T.getText().trim())); //trim(): 양끝의 공백을 제거
				dto.setY1(Integer.parseInt(y1T.getText().trim())); //trim(): 양끝의 공백을 제거
				dto.setY2(Integer.parseInt(y2T.getText().trim())); //trim(): 양끝의 공백을 제거
				dto.setZ1(Integer.parseInt(z1T.getText().trim())); //trim(): 양끝의 공백을 제거
				dto.setZ2(Integer.parseInt(z2T.getText().trim())); //trim(): 양끝의 공백을 제거
				
				//채우기
				dto.setFillyn(fill.isSelected());
...
///
		...
		can.addMouseMotionListener(new MouseAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				...
				x2T.setText(e.getX()+"");
				y2T.setText(e.getY()+"");
				can.repaint(); //=> paint된걸 Lis로 저장해줘야 함
				
///연필
				if(pen.isSelected()) {
					//드래그될때 바로 보관해야 함
					
					x1T.setText(x2T.getText()); //x
