...
public class Login extends JFrame implements WindowListener{
	...
	private JTextField idT, pwdT; //한줄씩만 들어가는 텍스트박스: textfield
...
//		idL.setSize(50, 30); //크기
		idL.setBounds(50, 80, 50, 20); //=> 위치와 크기를 한번에 설정
		idT.setBounds(120, 80, 100, 30);
		
		pwdL.setBounds(50, 120, 50, 20); //=> 위치와 크기를 한번에 설정
		...
		loginB.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if(idT.getText().equals("aa") && pwdT.getText().equals("aa")){
					new LoginResult("로그인 성공");
				}
				else {
					new LoginResult("로그인 실패");
				}
			}
		});
	}
